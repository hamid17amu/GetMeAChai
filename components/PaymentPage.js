    ...
    <div className="info flex justify-center items-center my-24 flex-col gap-2">
      ...
      <div className="paymen flex gap-3 w-[80%] mt-11">
        ...
        <div className="flex gap-2 flex-col">
          <input onChange={handleChange} name="message" value={paymentform.message} type="text" className='p-3 rounded-lg bg-slate-800' placeholder='Enter Message'required/>
          <input onChange={handleChange} name="name" value={paymentform.name} type="text" className='p-3 rounded-lg bg-slate-800' placeholder='Enter Name' required/>
          <input onChange={handleChange} name="amount" value={paymentform.amount} type="number" className='p-3 rounded-lg bg-slate-800' placeholder='Enter Amount' required/>
          <button onClick={()=>{pay(paymentform.amount * 100)}} disabled={paymentform.name?.length<3 || paymentform.message?.length<5 || !paymentform.amount} type="button" className="text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mb-2 disabled:from-slate-500">Pay</button>
        </div>
        {/* or choose Amount */}
